...
@section('title', 'Dashboard - ListUser')
@section('content')
    {!! Toastr::message() !!}
        ...
        <div class=" bg-putih w-full rounded-xl h-full pt-10 pb-16 px-6">
            <div class=" mb-4 flex justify-between">
                <div class=" text-hitam font-bold">
                    <h1>List User</h1>
                    <span class=" text-xs font-normal text-ketiga">!!!</span>
                </div>
                <div class=" flex items-center gap-2">
                    <form action="{{route('routeM.search')}}" method="POST">
                        @csrf
                        <div class=" flex items-center relative opacity-70">
                            <input type="text" name="search" placeholder='Cari User...' class=' border rounded-lg border-biru w-[200px] font-DMSans text-xs text-kedua placeholder:text-ketiga focus:placeholder:text-biru py-2 pl-4 focus:outline-none'/>
                ...
                <table class=" w-full flex flex-col gap-4 text-hitam">
                    <thead>
                        <tr class=" flex bg-kelima rounded-2xl px-10 py-3 text-xs font-bold">
                            <td class=" w-[50%]">No</td>
                            <td class=" w-[50%] -translate-x-4">Nama</td>
                            <td class=" w-[50%] translate-x-10">Username</td>
                            <td class=" w-[50%] translate-x-24">Telp</td>
                    ...
                    <tbody class=" flex flex-col gap-4 relative">
                        @foreach ($masyarakats as $item)
                            <tr class=" flex bg-putih border border-kelima rounded-lg px-10 py-4 text-[10px] items-center">
                                ...
                                <td class=" w-[50%] -translate-x-4">{{$item->nama}}</td>
                                <td class=" w-[50%] translate-x-10">{{$item->username}}</td>
                                <td class=" w-[50%] translate-x-24">{{$item->telp}}</td>
                                        ...
                                        <div class=" hidden absolute group-hover/2:flex z-10 rounded-md group-hover/2:bg-putih group-hover/2:drop-shadow-5xl top-full left-0 ">
                                            <div class=" flex flex-col items-start gap-3 py-2 px-3 w-full">
                                                <div class=" flex flex-col items-start gap-3 py-3 px-4 w-full">
                                                    <a href="{{route('routeM.show', $item->id)}}" class=" text-merah opacity-70 text-sm font-bold w-full hover:underline">
