            ...
            <div class=" flex gap-12 justify-between mt-6">
                <div class=" w-1/2 border border-kelima rounded-xl p-4 flex flex-col gap-6">
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        ...
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->judul}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Penulis <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->penulis}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Pengarang <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->pengarang}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Tahun Terbit <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->tahun_terbit}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Jumblah Halaman <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->jumblah_halaman}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Kategori <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->getDataBookP->kategori}}</h1>
                    </div>
                </div>
                <div class=" w-1/2 border border-kelima rounded-xl p-4 flex flex-col gap-6">
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Peminjam <span>:</span></h1>
                        <h1 class=" text-utama">Petugas <span class=" italic">{{$pborrowings->getDataPT->nama_petugas}}</span></h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Tanggal Peminjam <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->tgl_peminjaman}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Tanggal Pengembalian <span>:</span></h1>
                        <h1 class=" text-utama">{{$pborrowings->tgl_pengembalian}}</h1>
                    </div>
                    <div class="flex items-center justify-between border-b pb-3 border-b-kelima">
                        <h1>Status <span>:</span></h1>
                        @if ($pborrowings->getDataBookP->status == 'tersedia')
                        ...
                        @elseif($pborrowings->getDataBookP->status == 'dipinjam')
